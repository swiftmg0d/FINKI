...
public class MoviesController {
     ...
     */
    @GetMapping({"/","/movies"})
    public String showMovies(Model model, @RequestParam(required = false) Double rating, @RequestParam(required = false) Genre genre) {
        List<Movie> list0fMovies = null;
        if (rating == null && genre == null) {
            list0fMovies = this.movieService.listAllMovies();
        } else {
            list0fMovies = this.movieService.listMoviesWithRatingLessThenAndGenre(rating, genre);
        }
        model.addAttribute("movies", list0fMovies);
        model.addAttribute("genres", Arrays.asList(Genre.Action,Genre.Comedy,Genre.Horror));
        return "list";
    }
