...
if __name__ == '__main__':
    ...
    col_index_delete = int(input())

    dt_train_set, dt_test_set = divide_dataset(dataset)

    trainX, trainY = [trainX[:-1] for trainX in dt_train_set], [trainY[-1] for trainY in dt_train_set]
    testX, testY = [testX[:-1] for testX in dt_test_set], [testY[-1] for testY in dt_test_set]

    scaler = MinMaxScaler(feature_range=(-1, 1))
    clf = MLPClassifier(hidden_layer_sizes=hidden_layer_sizes, learning_rate_init=learning_rate_init, max_iter=20,random_state=0)

    trainX = scaler.fit_transform(trainX)
    clf.fit(trainX, trainY)

    train_accuracy = sum(x == y for x, y in zip(trainY, clf.predict(trainX)))

    test_accuracy = sum(x == y for x, y in zip(testY, clf.predict(testX)))

    entry = [entry for entry in input().split(" ")]

    if train_accuracy/len(trainY) >= test_accuracy/len(testY) * 1.15:
        print("Se sluchuva overfitting")
    else:
        print("Ne se sluchuva overfitting")
        print(clf.predict(scaler.transform([entry]))[0])
