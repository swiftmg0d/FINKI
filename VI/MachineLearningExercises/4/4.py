...
def divide_dataset(dataset):
    return [feature[:-1] for feature in dataset], [_class[-1] for _class in dataset]


def delete_column(data, index):
    return [[v for i, v in enumerate(list) if i != index] for list in data]


if __name__ == '__main__':
    index = int(input())

    dt_features, dt_classses = divide_dataset(dataset)
    dt_features = delete_column(dt_features, index)

    n_estimators = int(input())
    criterion = input()

    clf = RandomForestClassifier(random_state=0, n_estimators=n_estimators, criterion=criterion)

    trainX, trainY = dt_features[:int(len(dt_features) * 0.85)], dt_classses[:int(len(dt_classses) * 0.85)]
    testX, testY = dt_features[int(len(dt_features) * 0.85):], dt_classses[int(len(dt_classses) * 0.85):]

    clf.fit(trainX, trainY)

    accuracy = sum(x == y for x, y in zip(testY, clf.predict(testX)))


    entry=[entry for entry in input().split(" ")]
    entry=[x for i,x in enumerate(entry) if i!=index ]

    print(f"Accuracy: {accuracy / len(testY)}")
